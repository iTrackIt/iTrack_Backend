...
public class EducatorServiceImpl implements EducatorService {
    ...
    public EducatorServiceImpl(EducatorRepository educatorRepository, EducatorDTOMapper educatorDTOMapper, JwtService jwtService) {
...
//        SecurityContextHolder.getContext().setAuthentication(authentication);
        Educator educator = this.educatorRepository.findByEmail(email).orElseThrow(() -> new UsernameNotFoundException("User with email (" + email + ") does not exist"));
        if (educator == null) return null;
        boolean isMatch = encoder.isMatch(password, educator.getPassword());
        if (!isMatch) return null;
        System.out.println("User:  " + educator.getFullNames());
        String jwtToken = this.jwtService.generateToken(educator.getEmail());
        System.out.println("Token:  " + jwtToken);
        return new LoginResponse(jwtToken, educator);
    }

    @Override
    public Educator getLoggedInEducator(){
            String email;
            ...
            return educatorRepository.findByEmail(email).orElseThrow(() -> new UsernameNotFoundException("Logged in user not found"));
    }
